...
import { Loader2, Search, Activity, User as UserIcon } from 'lucide-react';

const ITEM_HEIGHT = 48; // compact density
const ITEM_HEIGHT_DETAILED = 84; 
...
const TaskRow = memo(({ 
  ...
  // Calculate total time safely based on computed fields or local metrics
  const totalTime = task.totalTime || task.totalTimeSpent || 0;
  
  const stageBreakdown = task.timeByStage?.slice(0, 4) || [];
  const userBreakdown = task.timeByUser?.slice(0, 3) || [];
  ...
  return (
    ...
    >
      <div 
        onClick={onClick}
        className={cn(
          "group flex items-center justify-between w-full h-full gap-4 px-5 rounded-2xl border border-surface-100/50 dark:border-surface-800/50 transition-all cursor-pointer bg-white shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] dark:bg-surface-900/60 hover:border-brand-300 dark:hover:border-brand-500/50 hover:shadow-md hover:-translate-y-0.5",
          !isDetailed && "py-2.5",
          isDetailed && "py-4 flex-col items-start h-auto"
        )}
      >
        <div className="flex w-full items-center justify-between min-w-0 pr-6 relative">
          ...
          <div className="flex items-center min-w-0 pr-4 w-[40%]">
            <span className={cn('w-2 h-2 rounded-full flex-shrink-0 mr-3')} style={{ backgroundColor: statusMeta.color }} />
          ...
          {/* Hover Expansion (Overlays via Portal) */}
          {isHovered && !isDetailed && createPortal(
             ...
             >
                ...
                ) : <p className="text-[11px] text-surface-400 italic">No activity recorded for this task yet.</p>}
             </div>,
             document.body
          )}
          ...
          <div className="hidden md:flex flex-1 items-center gap-1.5 pr-4 justify-end">
             {userBreakdown.length > 0 ? userBreakdown.map(ut => {
          ...
          <div className="flex items-center gap-2 w-[120px] justify-end flex-shrink-0">
             ...
             <span className={cn(
               "ml-3 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide rounded-lg border whitespace-nowrap",
...
export const HighDensityTaskList: React.FC<HighDensityTaskListProps> = ({ projectId, categoryId, onOpenTask }) => {
  ...
  const fetchTasks = useCallback(async (pageNum: number, isInitial = false) => {
    try {
      ...
      const totalTasks = res.data?.meta?.total || 0;
      ...
      if (isInitial) {
        ...
        setTasks(prev => {
          ...
          const unique = finalBatch.filter((t: Task) => !existingIds.has(t.id));
          return [...prev, ...unique];
        });
      }
  ...
  const onRowsRendered = useCallback(({ stopIndex }: any) => {
    if (!loading && !loadingMore && hasMore && stopIndex >= tasks.length - 10) {
      fetchTasks(page + 1);
    }
  }, [loading, loadingMore, hasMore, tasks.length, page, fetchTasks]);

  if (loading && tasks.length === 0) {
    return (
      <div className="flex h-[300px] flex-col items-center justify-center gap-3">
        <Loader2 size={24} className="animate-spin text-brand-500" />
        <p className="text-xs text-surface-400 font-medium">Lazy loading high-density task list...</p>
      </div>
    );
  }
  ...
  const currentItemHeight = isDetailed ? ITEM_HEIGHT_DETAILED : ITEM_HEIGHT;

  return (
    <div className="flex flex-col h-[700px] border border-surface-200 dark:border-surface-800 rounded-3xl bg-surface-50/30 dark:bg-surface-900/10 overflow-hidden relative">
      <div className="flex items-center justify-between px-6 py-3 border-b border-surface-200 dark:border-surface-800 bg-white/50 dark:bg-surface-950/50 backdrop-blur-sm z-10">
         <div>
           ...
           <p className="text-[10px] text-surface-500 font-medium mt-0.5">Showing {tasks.length} {hasMore && `out of ${total}`} tasks</p>
         </div>
      </div>

      <div className="flex-1 w-full relative">
        <List
          style={{ height: 650, width: '100%', overflowY: 'auto' }}
          className="custom-scrollbar"
          rowCount={tasks.length}
          rowHeight={currentItemHeight}
          ...
          rowComponent={({ index, style }) => (
            <TaskRow 
              ...
              onClick={() => onOpenTask(tasks[index])} 
            />
          )}
        />

        {loadingMore && (
           <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 dark:bg-surface-800/90 shadow px-4 py-2 rounded-full border border-surface-200 dark:border-surface-700 flex items-center gap-2">
             <Loader2 size={12} className="animate-spin text-brand-500" />
             <span className="text-[10px] uppercase font-bold tracking-widest text-surface-500">Loading {page * 50}...</span>
           </div>
        )}
      </div>
    </div>
